...
#include <linux/input/unified_driver_4/lgtp_project_setting.h>

#if defined(TOUCH_PLATFORM_MSM8936)
#include <mach/board_lge.h>
#elif defined(TOUCH_PLATFORM_MSM8937)
...
typedef struct TouchDeviceControlFunctionTag {
	...
	int (*DoSelfDiagnosis)(int *pRawStatus, int *pChannelStatus, char *pBuf, int bufSize, int *pDataLen);
    int (*DoSelfDiagnosis_Lpwg)(int *lpwgStatus, char *pBuf, int bufSize, int *pDataLen);
	void (*NotifyHandler)(TouchNotify notify, int data);
	int (*MftsControl)(TouchDriverData *pDriverData);
