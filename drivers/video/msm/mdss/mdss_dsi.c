...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (ctrl_pdata->lge_extra.extra_power_state == 0) {
		ret = msm_dss_enable_vreg(
		...
		ctrl_pdata->lge_extra.extra_power_state =1;
	}
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_COMMON)
	LGE_OVERRIDE_VALUE(esc_clk_rate, ctrl_pdata->lge_extra.xo_clk_rate);
#endif
...
#if defined(CONFIG_LGE_MIPI_DSI_LGD_K7J_FHD_VIDEO_INCELL_LCD_PANEL)
#if IS_ENABLED(CONFIG_LGE_DISPLAY_RECOVERY_ESD)
		if (esd_detected == 0)
#endif
...
#if defined(CONFIG_TOUCHSCREEN_UNIFIED_DRIVER_3) || defined(CONFIG_LGE_TOUCH_CORE)
#if !defined(CONFIG_LGE_MIPI_DSI_LGD_K7J_FHD_VIDEO_INCELL_LCD_PANEL)
		touch_notifier_call_chain(LCD_EVENT_TOUCH_LPWG_OFF,NULL);
#endif
#endif
...
#endif
			touch_notifier_call_chain(LCD_EVENT_TOUCH_LPWG_ON, NULL);
#elif defined(CONFIG_TOUCHSCREEN_UNIFIED_DRIVER_3) || defined(CONFIG_LGE_TOUCH_CORE)
		if (lge_get_panel_type() != PH2_SHARP) {
			touch_notifier_call_chain(LCD_EVENT_TOUCH_LPWG_ON,NULL);
		}
#endif
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_COMMON)
					ctrl_pdata->lge_extra.xo_clk_rate?ctrl_pdata->lge_extra.xo_clk_rate:XO_CLK_RATE,
#else
					19200000,
#endif
					MDSS_DSI_CLK_UPDATE_CLK_RATE_AT_ON);
