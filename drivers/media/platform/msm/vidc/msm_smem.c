	...
	int map_kernel)
{
	...
	if (is_iommu_present(client->res)) {
		...
		dprintk(VIDC_DBG,
			...
			size, align);
		heap_id_mask = ION_HEAP(ION_ADSP_HEAP_ID);
	}
	...
	trace_msm_smem_buffer_ion_op_start("ALLOC", (u32)buffer_type,
		heap_id_mask, size, align, flags, map_kernel);
	hndl = ion_alloc(client->clnt, size, align, heap_id_mask, ion_flags);
	if (IS_ERR_OR_NULL(hndl)) {
		dprintk(VIDC_ERR,
