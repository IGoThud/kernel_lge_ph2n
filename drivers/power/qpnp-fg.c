...
#define COMP_FACTOR             330     /* -1.7degree/1A */
...
static int calc_tuned_temp(int temp)
{
	...
	pr_info("comp_factor = %d, org temp : %d, delta temp : %d, report temp : %d\n",
			COMP_FACTOR,temp, delta_temp, (temp + delta_temp));
...
static int fg_config_imptr_pulse(struct fg_chip *chip, bool slow)
{
	...
	if (slow == chip->imptr_pulse_slow_en) {
		if (fg_debug_mask & FG_STATUS)
			pr_debug("imptr_pulse_slow is %sabled already\n",
				slow ? "en" : "dis");
		return 0;
	}
...
static int fg_config_esr_extract(struct fg_chip *chip, bool disable)
{
	...
	if (disable == chip->esr_extract_disabled) {
		if (fg_debug_mask & FG_STATUS)
			pr_debug("ESR extract already %sabled\n",
				disable ? "dis" : "en");
		return 0;
	}
	...
	if (fg_debug_mask & FG_STATUS)
		pr_debug("ESR extract is %sabled\n", disable ? "dis" : "en");
done:
	fg_mem_release(chip);
	return rc;
}
...
static void discharge_gain_work(struct work_struct *work)
{
	...
	if (fg_debug_mask & FG_STATUS)
		pr_debug("Capacity: %d, medc_gain: %lld highc_gain: %lld\n",
	...
	if (rc)
		...
		pr_debug("Value [%x %x] written to ki_coeff_medc\n", buf[0],
			buf[1]);
	...
	if (rc)
		...
		pr_debug("Value [%x %x] written to ki_coeff_highc\n", buf[0],
			buf[1]);
...
static int fg_batt_profile_init(struct fg_chip *chip)
{
 ...
 wait:
...
#ifdef CONFIG_LGE_PM_BATTERY_ID_CHECKER
#ifndef CONFIG_MACH_MSM8917_LV7_TRF_US
#ifndef CONFIG_MACH_MSM8917_LV7_TRF_US_VZW
	if (!fg_batt_type)
...
#if defined(CONFIG_MACH_MSM8917_LV7_TRF_US) || defined(CONFIG_MACH_MSM8917_LV7_TRF_US_VZW)
		if (!fg_batt_type){
			update_battery_type(&fg_batt_type);
			chip->batt_type = fg_batt_type;
			profile_node = of_batterydata_get_best_profile(batt_node, "bms",
	...
	if (chip->use_thermal_coefficients && strstr(fg_batt_type, "Tocad")) {
...
static int fg_common_hw_init(struct fg_chip *chip)
{
	...
	if (!chip->sw_rbias_ctrl) {
		rc = fg_mem_masked_write(chip, EXTERNAL_SENSE_SELECT,
					 BATT_TEMP_CNTRL_MASK,
#ifdef CONFIG_LGE_PM
					 FORCE_RBIAS_ON_BIT | TEMP_SENSE_ALWAYS_BIT,
#else
