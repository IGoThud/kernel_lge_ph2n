						...
						enum power_supply_type type)
{
	...
	if (chip->typec_psy && (type != POWER_SUPPLY_TYPE_USB))
		...
		if(chip->qc20.current_status == QC20_CURRENT_LIMMITED)
			current_limit_ma = chip->qc20.iusb[QC20_CURRENT_LIMMITED];
		else
			current_limit_ma = chip->qc20.iusb[QC20_CURRENT_NORMAL];
	}
#else
	else if (type == POWER_SUPPLY_TYPE_USB_HVDCP)
	...
	if(chip->qc20.is_qc20){
		if(chip->qc20.current_status == QC20_CURRENT_LIMMITED){
			pr_smb(PR_LGE, "[QC20] Set qc20.iusb: %d, qc20.ibat: %d, "
				...
				current_limit_ma, chip->qc20.ibat[QC20_CURRENT_LIMMITED],
				smbchg_iusb_hvdcp_limit, smbchg_ibat_limit);

			rc = vote(chip->fcc_votable, BATT_TYPE_FCC_VOTER, true,
				chip->qc20.ibat[QC20_CURRENT_LIMMITED]);
	...
	struct kernel_param *kp)
{
	...
	if (ret) {
		...
		return 0;
	}else if(quick_charging_state < 0){
	...
	pr_smb(PR_LGE, "[QC20] quick_charging_state: %d, qc20.status: %d,"
		" qc20.is_qc20: %d \n",
		quick_charging_state, the_chip->qc20.status, the_chip->qc20.is_qc20);

	if (the_chip->qc20.status)
		the_chip->qc20.current_status = QC20_CURRENT_LIMMITED;
	else
		the_chip->qc20.current_status = QC20_CURRENT_NORMAL;

	if(the_chip->qc20.is_qc20){
		if(the_chip->qc20.current_status == QC20_CURRENT_LIMMITED){
			pr_smb(PR_LGE, "[QC20] Set qc20.iusb: %d, qc20.ibat: %d, "
				"iusb thermal: %d, ibat thermal: %d\n",
				the_chip->qc20.iusb[QC20_CURRENT_LIMMITED],
				the_chip->qc20.ibat[QC20_CURRENT_LIMMITED],
				smbchg_iusb_hvdcp_limit, smbchg_ibat_limit);

			ret = vote(the_chip->usb_icl_votable, PSY_ICL_VOTER,
				true, the_chip->qc20.iusb[QC20_CURRENT_LIMMITED] );
			ret |= vote(the_chip->fcc_votable, BATT_TYPE_FCC_VOTER,
				true, the_chip->qc20.ibat[QC20_CURRENT_LIMMITED] );
	...
	struct kernel_param *kp)
{
	...
	if(the_chip->qc20.is_qc20){
		if(the_chip->qc20.current_status == QC20_CURRENT_LIMMITED){
			pr_smb(PR_LGE, "[QC20] Skip thermal settings. "
				"(hvdcp: present, state: QC20_CURRENT_LIMMITED)\n");
			return 0;
		}
	}
...
static void smbchg_monitor_batt_temp(struct work_struct *work)
{
	...
	if(chip->first_soc_est_done != true){
		chip->bms_psy->get_property(chip->bms_psy,
			POWER_SUPPLY_PROP_FIRST_SOC_EST_DONE, &ret);
		chip->first_soc_est_done = ret.intval;
...
#ifdef CONFIG_LGE_PM_QC20_SCENARIO
	if (lge_get_boot_mode() != LGE_BOOT_MODE_CHARGERLOGO){
		chip->qc20.current_status = QC20_CURRENT_LIMMITED;
...
#ifdef CONFIG_LGE_PM
	if(!chip->hvdcp_not_supported) {
		pr_info("HVDCP enable check warm reset\n");
