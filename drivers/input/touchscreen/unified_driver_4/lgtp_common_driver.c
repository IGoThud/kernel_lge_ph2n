...
static void TouchIrqEventHandler(TouchDriverData *pDriverData)
{
	...
	case DATA_KNOCK_ON:
		if (pDriverData->currState == STATE_KNOCK_ON_ONLY || pDriverData->currState == STATE_KNOCK_ON_CODE) {
			...
			TOUCH_WARN("Unmatched event from device ( event = KNOCK_ON ) state = %d\n", pDriverData->currState);
		}
        break;
...
#if !defined(TOUCH_MODEL_PH1) && !defined(TOUCH_MODEL_PH2)
...
static void async_touch_init(void *data, async_cookie_t cookie)
{
	...
	if(lge_get_laf_mode() == LGE_LAF_MODE_LAF) {
		TOUCH_LOG("LGE_LAF_MODE_LAF, not register touch-driver\n");
		gpio_direction_output(TOUCH_GPIO_RESET, 0);
		msleep(2);
		TOUCH_LOG("LGE_LAF_MODE_LAF, Touch reset pin down(LOW)\n");
		goto exit_async_touch_init_fail;
	}
